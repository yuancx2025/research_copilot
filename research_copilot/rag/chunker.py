...
class Chunker:
    """Parent-child chunking strategy for documents."""
    ...
    def create_chunks(self, path_dir=config.MARKDOWN_DIR):
        """Create chunks from all markdown files in directory."""
    ...
    def create_chunks_single(self, md_path):
        """Create chunks from a single markdown file."""
    ...
    def create_chunks_from_text(self, text: str, source_metadata: dict = None):
        """
        Create chunks from raw text (for multi-source indexing).
        
        Args:
            text: Raw text content to chunk
            source_metadata: Metadata to add to all chunks (source_type, source_id, etc.)
        
        Returns:
            Tuple of (parent_chunks, child_chunks)
        """
        parent_chunks = self.__parent_splitter.split_text(text)
        ...
        # Create a dummy path for metadata purposes
        dummy_path = Path(source_metadata.get("source_id", "unknown") if source_metadata else "unknown")
        
        for i, p_chunk in enumerate(cleaned_parents):
            parent_id = f"{dummy_path.stem}_parent_{i}"
            p_chunk.metadata.update({
                "source": source_metadata.get("source_id", "unknown") if source_metadata else "unknown",
                "parent_id": parent_id
            })
            
            # Add source-specific metadata
            if source_metadata:
                p_chunk.metadata.update(source_metadata)
