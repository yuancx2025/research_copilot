...
import pytest
from research_copilot.runtime.mcp.adapter import MCPToolAdapter
from research_copilot.runtime.mcp.client import MCPConnectionError, AuthorizationRequired
...
async def test_cache_schema_and_disconnect():
    ...
    with patch('research_copilot.runtime.mcp.adapter.create_client', return_value=client):
...
async def test_discovery_errors(error, expected):
    with patch('research_copilot.runtime.mcp.adapter.create_client') as create:
